    ...
    bool Scanning::DoFocus(TFile* f_rootfile, PSTCT *stct, analysis* ana) {
        ...
        for(int j=0;j<=numO;j++)
           {
             switch(optic_axis)
           {
           case 0: wf[j]=stct->Projection(ChNumber,scanning_axis,j,0,0,0,0,numS); break;
           case 1: wf[j]=stct->Projection(ChNumber,scanning_axis,0,j,0,0,0,numS); break;
           case 2: wf[j]=stct->Projection(ChNumber,scanning_axis,0,0,j,0,0,numS); break;
           }
        ...
        if(ana->CH_PhDiode()) {
            ...
            for(int j=0;j<=numO;j++)
            {
                switch(optic_axis)
                {
                case 0: wf1[j]=stct->Projection(ana->CH_PhDiode()-1,scanning_axis,j,0,0,0,0,numS); break;
                case 1: wf1[j]=stct->Projection(ana->CH_PhDiode()-1,scanning_axis,0,j,0,0,0,numS); break;
                case 2: wf1[j]=stct->Projection(ana->CH_PhDiode()-1,scanning_axis,0,0,j,0,0,numS); break;
                }
        ...
        for(int j=0;j<=numO;j++){
            ...
            minQ[j] = ff2->Eval(ff2->GetParameter(0) + ff2->GetParameter(3)/2);
        ...
        if(ana->CH_PhDiode()) {
            ...
            for(int j=0;j<=numO;j++) {
                ...
                for(int i=0;i<=numS;i++) {
                    temp_Y3[i] = temp_Y1[i]/temp_Y2[i];
                }
                cc_norm[j] = new TGraph(numS,cc[0]->GetX(),temp_Y3);
                cc_norm[j]->Fit("ff2","Rq");
                gStyle->SetOptFit(1);

                width_new[j]=ff2->GetParameter(1)*2.35/TMath::Sqrt(2);
                pos_new[j]=ff2->GetParameter(0)+ ff2->GetParameter(3)/2;
                minQ_new[j] = ff2->Eval(ff2->GetParameter(0) + ff2->GetParameter(3)/2);
            }
        }
        ...
        MINQg->GetHistogram()->GetYaxis()->SetTitle("min Q from fit [arb]");
        ...
        if(ana->CH_PhDiode()) {
            ...
            MINQg1->GetHistogram()->GetYaxis()->SetTitle("min Q from fit [arb]");
...
/*
        ...
        TGraph *CORg=new TGraph(numO+1,width,minQ);
