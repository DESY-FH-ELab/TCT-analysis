...
/// Analysis manager
        ...
        if(config->DO_TopDepletion() && config->TCT_Mode()==0)
        {
            if(DoTopDepletion()) std::cout<<"-- Depletion Votage search finished succesfully!"<<std::endl;
#ifdef USE_GUI
            progress->setValue(progress->value()+1);
#endif
        }

        if(config->DO_TopMobility() && config->TCT_Mode()==0)
        {
            if(DoTopMobility()) std::cout<<"-- Mobility search finished succesfully!"<<std::endl;
#ifdef USE_GUI
            progress->setValue(progress->value()+1);
#endif
        }
...
/// Top-TCT Depletion Voltage Calculation Method
    bool Scanning::DoTopDepletion() {

        if(!CheckTopDepletion()) {std::cout<<"No data for depletion voltage search. Skipping..."<<std::endl; return false;}
...
/// Top-TCT Charge Carriers Mobility Calculation Method
    bool Scanning::DoTopMobility() {

        if(!CheckTopMobility()) {std::cout<<"No data for mobility search. Skipping..."<<std::endl; return false;}
...
/// Check before depletion voltage search
    bool Scanning::CheckTopDepletion() {

        std::cout<<"\t- DO Top Depletion: "<< config->DO_TopDepletion() <<std::endl;
        ...
        std::cout<<"TopDepletionVoltage Data Test Passed. Processing..."<<std::endl;
        return true;
    }
/// Check before depletion voltage search
    bool Scanning::CheckTopMobility() {

        std::cout<<"\t- DO Top Charge Carriers Mobility: "<< config->DO_TopDepletion() <<std::endl;
        ...
        if(NSource>=7) std::cout<<"\t\t\tVoltage scan contains "<<NSource<<" points. OK"<<std::endl;
            ...
            std::cout<<"\t\t\tVoltage scan contains only "<<NSource<<" points. Not enough for mobility search."<<std::endl;
            return false;
        }

        std::cout<<"TopMobility Data Test Passed. Processing..."<<std::endl;
        return true;
    }
/// Check before depletion voltage search
